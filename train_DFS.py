...
def train_dfs(opt): 
    fold = opt.fold
    print('\n U-Net with DFS training: Fold:', fold)
    # Creating or loading the model 
    model = Optim_U_Net(img_ch=opt.input_nc,output_ch=opt.output_nc, USE_DS = False, USE_DFS = True)
    if(opt.load_model):
    ...
    for EPOCH in range(train_epoch):
        ...
        for _, t_batch_num in enumerate(train_data_LD):
            ...
            t_gim_sub = g_data_cell(gpath_cell, t_batch_num, width, height, USE_DS=False)
            ...
            loss = loss_func2(OUTPUT6, L.make_one_hot(target6,2).cuda()) + loss_func2(OUTPUT5, L.make_one_hot(target5,2).cuda()) + loss_func2(OUTPUT4, L.make_one_hot(target4,2).cuda()) + loss_func2(OUTPUT3, L.make_one_hot(target3,2).cuda()) + loss_func2(OUTPUT, L.make_one_hot(target,2).cuda()) 
            optimizer.zero_grad()              
            loss.backward()                     
            optimizer.step()
        ...
        for _, v_batch_num in enumerate(valid_data_LD):
            ...
            # computing loss
            loss_v = loss_func2(OUTPUT6, L.make_one_hot(target6,2).cuda()) + loss_func2(OUTPUT5, L.make_one_hot(target5,2).cuda()) + loss_func2(OUTPUT4, L.make_one_hot(target4,2).cuda()) + loss_func2(OUTPUT3, L.make_one_hot(target3,2).cuda()) + loss_func2(OUTPUT, L.make_one_hot(target,2).cuda()) 
...
def test_dfs(opt):
    fold = opt.fold
    print('\n U-Net with DFS testing: Fold:', fold)
    # Creating or loading the model 
    model = Optim_U_Net(img_ch=opt.input_nc,output_ch=opt.output_nc, USE_DS = False, USE_DFS = True)
    if(opt.load_model):
        model.load_state_dict(torch.load(opt.modelpath))
